...
var runStyles = require("./run-styles");
...
function DocumentConversion(options, comments) {
    ...
    function convertParagraph(element, messages, options) {
        var childOptions = _.extend({}, options, {
            paragraphRightToLeft: element.isRightToLeft
        });
        var nodes = htmlPathForParagraph(element, messages).wrap(function() {
            var content = convertElements(element.children, messages, childOptions);
        ...
        return applyBlockAttributes(nodes, bidi.paragraphAttributes(element));
    }

    function applyBlockAttributes(nodes, attributes) {
        ...
        return nodes.map(function(node) {
            ...
            var tag = htmlPaths.element(
                node.tag.tagName,
                mergeElementAttributes(node.tag.attributes, attributes),
                {fresh: node.tag.fresh, separator: node.tag.separator}
            );
            return Html.elementWithTag(tag, node.children);
        });
    }

    function mergeElementAttributes(existing, extra) {
        var merged = _.extend({}, existing, extra);
        if (existing.style && extra.style) {
            merged.style = existing.style + ";" + extra.style;
        }
        return merged;
    }
    ...
    function convertRun(run, messages, options) {
        ...
        var paths = [];
        var highlightPath = run.highlight !== null
            ? findHtmlPath({type: "highlight", color: run.highlight})
            : null;
        if (highlightPath) {
        ...
        var hasCustomUnderline = run.isUnderline && (run.underlineStyle || run.underlineColor);
        if (run.isUnderline && !hasCustomUnderline) {
        ...
        var colorPath = run.color ? findHtmlPath({type: "color", val: run.color}) : null;
        var shadingPath = run.backgroundColor ? findHtmlPath({type: "shading", val: run.backgroundColor}) : null;
        if (colorPath) {
            paths.push(colorPath);
        }
        if (shadingPath) {
            paths.push(shadingPath);
        }

        var inlineStyle = runStyles.buildRunInlineStyle(run, {
            hasHighlightStyleMap: !!highlightPath,
            hasColorStyleMap: !!colorPath,
            hasShadingStyleMap: !!shadingPath
        });
        var directionAttributes = bidi.runDirectionAttributes(
            run,
            options.paragraphRightToLeft
        );
        var spanAttributes = {};
        if (inlineStyle) {
            spanAttributes.style = inlineStyle;
        }
        if (directionAttributes) {
            _.extend(spanAttributes, directionAttributes);
        }
        if (!_.isEmpty(spanAttributes)) {
            paths.push(htmlPaths.element("span", spanAttributes, {fresh: false}));
        }
    ...
    function convertTableCell(element, messages, options) {
        ...
        if (element.backgroundColor) {
            attributes.style = "background-color:" + element.backgroundColor;
        }
