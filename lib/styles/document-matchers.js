...
function color(options) {
    return new ColorMatcher(options);
}

function shading(options) {
    return new ShadingMatcher(options);
}
...
function ColorMatcher(options) {
    options = options || {};
    this._val = options.val;
}

ColorMatcher.prototype.matches = function(element) {
    return element.type === "color" &&
        (this._val === undefined || normalizeColorValue(element.val) === normalizeColorValue(this._val));
};

function ShadingMatcher(options) {
    options = options || {};
    this._val = options.val;
}

ShadingMatcher.prototype.matches = function(element) {
    return element.type === "shading" &&
        (this._val === undefined || normalizeColorValue(element.val) === normalizeColorValue(this._val));
};
